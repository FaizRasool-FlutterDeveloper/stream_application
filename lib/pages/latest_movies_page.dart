...
class LatestMoviesPage extends GetView<LatestMoviesController> {
  const LatestMoviesPage({super.key});
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Latest Movies')),
      body: Obx(() {
        if (controller.isLoading.value) return const Center(child: CircularProgressIndicator());
        final movies = controller.latestMovies;
        if (movies.isEmpty) return const Center(child: Text('No latest movies.'));
        return Column(children: [
          Padding(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            const Text('Latest Movies', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
            TextButton(onPressed: () => Get.toNamed(AppRoutes.seeMore, arguments: {'title': 'Latest Movies', 'movies': movies}), child: const Text('See More'))
          ])),
